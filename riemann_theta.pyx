...
def oscillatory_part(z, Omega, mode, epsilon, derivs, accuracy_radius, axis):
    ...
    z = numpy.array(z, dtype=numpy.complex128)
    if len(z.shape) == 1:
        num_vectors = 1
        g = len(z)
    else:
    ...
    Omega = numpy.array(Omega, dtype=numpy.complex128)
    Y = Omega.imag
    ...
    if isinstance(derivs, list):
        ...
        values = numpy.zeros(num_vectors, dtype=numpy.complex128)
        ...
        # get the derivatives
        if len(derivs):
            derivs = numpy.array(derivs, dtype=numpy.complex128).flatten()
        ...
        for k in range(num_vectors):
            values[k] = numpy.complex128(real[k] + 1.0j*imag[k])
        ...
        output = numpy.zeros(shape=(derivs.shape[0], num_vectors), dtype=numpy.complex128)
        ...
        if mode == 0:
            ...
            for i, value in enumerate(derivs):
                ...
                if len(value):
                    ...
                    value = numpy.array(value, dtype=numpy.complex128).flatten()
                    nderivs = len(value) / g
            ...
            inderivs = numpy.array([item for sublist in derivs for item in sublist], dtype=numpy.complex128)
            ...
            for i in range(derivs.shape[0]):
                    if num_vectors == 1:
                        output[i] = numpy.complex128(real[i] + 1.0j*imag[i])
                    else:
                        for k in range(num_vectors):
                            output[i, k] = numpy.complex128(real[k+i*num_vectors] + 1.0j*imag[k+i*num_vectors])
...
def normalized_oscillatory_part(z, Omega, mode, epsilon, derivs, accuracy_radius, axis):
    ...
    if isinstance(derivs, list):
        ...
        if len(derivs):
            ...
            # compute the finite sum for each z-vector
            if(mode==0):
                values_nom = numpy.zeros(num_vectors, dtype=numpy.complex128)
                values_den = numpy.zeros(num_vectors, dtype=numpy.complex128)
                ...
                for k in range(num_vectors):
                    values_nom[k] = numpy.complex128(real[k] + 1.0j*imag[k])

                finite_sum_without_derivatives(real, imag,
                                               &x[0], &y[0], &X[0,0],&Yinv[0,0], &T[0,0], &S[0,0],g, N, num_vectors)
                for k in range(num_vectors):
                    values_den[k] = numpy.complex128(real[k] + 1.0j*imag[k])
...
cdef class RiemannTheta_Function(object):
    ...
    def oscillatory_part_gradient(self, z, Omega, mode=0, epsilon=1e-8,
        ...
        Omega = numpy.array(Omega, dtype=numpy.complex128)
        g = Omega.shape[0]
        gradients = numpy.zeros_like(z, dtype=numpy.complex128)
    ...
    def oscillatory_part_hessian(self, z, Omega,mode=0, epsilon=1e-8,
        ...
        if len(z.shape) == 1:
            ...
            n = z.shape[(axis+1) % 2]
        hessians = numpy.zeros((n,g,g), dtype=numpy.complex128)
